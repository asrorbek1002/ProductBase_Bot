...
async def product_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        ...
        text="Mahsulot sotib olingan narxini kiriting:",
        reply_markup=InlineKeyboardMarkup(back_main_menu_buttons())
    )
...
async def product_purchase_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await context.bot.send_message(chat_id=update.effective_user.id, text="Iltimos, to'g'ri narx kiriting:")
        return PURCHASE_PRICE
    
    await context.bot.send_message(
        chat_id=update.effective_user.id, 
        text="Mahsulot sotish narxini kiriting:",
        reply_markup=InlineKeyboardMarkup(back_main_menu_buttons())
    )
...
async def product_selling_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(chat_id=update.effective_user.id, text="Mahsulot miqdorini kiriting:")
    return QUANTITY
...
async def product_quantity(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await context.bot.send_message(chat_id=update.effective_user.id, text="Iltimos, Miqdorni to'g'ri kiriting:")
        return QUANTITY
...
async def product_unit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not categories:
        await query.message.reply_text("Hali kategoriyalar mavjud emas. Mahsulot qo'shishdan avval kategoriya qo'shganingiz maqul ☺️")
        return ConversationHandler.END
    
    keyboard = [[InlineKeyboardButton(cat.name, callback_data=str(cat.id))] for cat in categories]
    reply_markup = InlineKeyboardMarkup(keyboard + back_main_menu_buttons())
    await query.message.reply_text("Kategoriya tanlang:", reply_markup=reply_markup)
    return CATEGORY
...
async def product_category(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await context.bot.send_message(chat_id=update.effective_user.id, text="Kategoriyani topib bo'lmadi. Qayta urinib ko'ring.")
        return CATEGORY
